...
Response::Response(): is_finished(-1), status_code(-1), is_cgi(-1), port(-1), byte_sent(0), host(""), body(""), to_fetch(""), file_path(""), exec_path(""), location_name("")
{}
...
void	Response::get_error_response(Server_info server, Location location)
{
	...
	body = "<!DOCTYPE html>\
	<style>    @import url('https://fonts.googleapis.com/css?family=Press Start 2P');html, body {  width: 100%;  height: 100%;  margin: 0;}* {  font-family: 'Press Start 2P', cursive;  box-sizing: border-box;}#app {  padding: 1rem;  background: black;  display: flex;  height: 100%;  justify-content: center;  align-items: center;  color: #fcff5b;  text-shadow: 0px 0px 10px;  font-size: 6rem;  flex-direction: column;}#app .txt {  }@keyframes blink {  0% {    opacity: 0;  }  49% {    opacity: 0;  }  50% {    opacity: 1;  }  100% {    opacity: 1;  }}.blink {  animation-name: blink;  animation-duration: 1s;  animation-iteration-count: infinite;}</style>\
	<html>\
		<head>\
...
void Response::get_auto_index_page_response(std::string dir_path)
{
	...
	status_code = 200;
	body = "<!DOCTYPE html>\
	<style>    @import url('https://fonts.googleapis.com/css?family=Press Start 2P');html, body {  width: 100%;  height: auto;  margin: 0;}* {  font-family: 'Press Start 2P', cursive;  box-sizing: border-box;}#app {  padding: 1rem;  background: black;  display: flex;  height: 100%;  justify-content: center;  align-items: center;  color: #fcff5b;  text-shadow: 0px 0px 5px;  font-size: 2rem;  flex-direction: column;}#app .txt {  font-size: 1.5rem;}@keyframes blink {  0% {    opacity: 0;  }  49% {    opacity: 0;  }  50% {    opacity: 1;  }  100% {    opacity: 1;  }}.blink {  animation-name: blink;  animation-duration: 1s;  animation-iteration-count: infinite;}	a:link {color: #5bdbff;} a:visited {color: #347e92;} a:hover {color: white;}\
	</style>\
	<html>\
		<head>\
			<meta http-equiv=content-type content=text/html; charset=UTF-8>\
		...
		<body id=app>\
		<h1 style='padding:1em; '>Index of "+ join_paths(location_name, to_fetch) + "</h1><hr><table class=txt style='width: 100%;	height: auto'><tr><th class=txt style='padding: 1em 3em 0em 2em;  text-align: left;'>Name<span class=blink>_</span></th><th class=txt style='padding: 1em 3em 0em 2em; '>Last modification date<span class=blink>_</span></th><th class=txt style='padding: 1em 3em 0em 2em;  text-align: right;'>Size in bytes<span class=blink>_</span></th></tr>";
	while ((element = readdir(dir)) != NULL)
	{
		body = body + "<tr><th class=txt style='padding: 1em 3em 0em 2em; color: white; text-align: left;'><a href=" + element->d_name + ((check_path(dir_path + element->d_name) % 2) ? "" : "/") +">" + \
		element->d_name + ((check_path(dir_path + element->d_name) % 2) ? "" : "/") + "</a></th><th class=txt style='padding: 1em 3em 0em 2em; color: white;'>" + \
		get_modification_date(dir_path + element->d_name) + "</th><th class=txt style='padding: 1em 3em 0em 2em; color: white; text-align: right;'>"+ get_file_size(dir_path + element->d_name) + "</th></tr>";
	}	
